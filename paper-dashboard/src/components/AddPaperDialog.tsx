...
export function AddPaperDialog() {
  ...
  useEffect(() => {
    if (editingPaper) {
      ...
      setExtraLinks(editingPaper.links.filter((l) => l.category !== 'repository' && l.category !== 'submission'));
  ...
  const handleSave = () => {
    const links: Link[] = [];
    if (portalUrl) links.push({ label: `${journal || 'Journal'} Portal`, url: portalUrl, category: 'submission' });
  ...
  // Author helpers
  const addAuthor = () => setAuthors([...authors, { name: '', role: 'co-author' }]);
  ...
  // Deliverable helpers
  const addDeliverable = () => setDeliverables([...deliverables, { name: '', type: 'manuscript', status: 'pending' }]);
      ...
      >
        ...
        <div style={{ padding: 20 }}>
            ...
            <div style={{ marginBottom: 8 }}>
              {authors.map((a, i) => (
                <div key={i} style={{ display: 'flex', gap: 4, marginBottom: 4, alignItems: 'center' }}>
                  ...
                  <input style={{ ...smallInputStyle, flex: 2 }} value={a.affiliation ?? ''} onChange={(e) => updateAuthorField(i, 'affiliation', e.target.value)} placeholder="Affiliation" />
              ...
              <button onClick={addAuthor} style={addBtnStyle}>+ Add Author</button>
            </div>
          )}
          ...
          <div style={sectionHeaderStyle} onClick={() => toggleSection('costs')}>
            <span>Costs & Funding</span>
            <span style={{ fontSize: 14 }}>{isExpanded('costs') ? '\u25B2' : '\u25BC'}</span>
          </div>
          {isExpanded('costs') && (
            <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                <label style={{ ...labelStyle, fontSize: 10 }}>APC Estimate</label>
          ...
          <div style={sectionHeaderStyle} onClick={() => toggleSection('tasks')}>
            <span>Co-author Tasks ({coauthorTasks.length})</span>
          ...
          {isExpanded('tasks') && (
            <div style={{ marginBottom: 8 }}>
              {coauthorTasks.map((t, i) => (
                <div key={i} style={{ display: 'flex', gap: 4, marginBottom: 4, alignItems: 'center' }}>
                  <input style={{ ...smallInputStyle, flex: 1 }} value={t.authorName} onChange={(e) => updateTaskField(i, 'authorName', e.target.value)} placeholder="Author" />
          ...
          {isExpanded('limits') && (
            <div style={{ display: 'flex', gap: 12, marginBottom: 8 }}>
              <div style={{ flex: 1 }}>
                ...
                <input style={smallInputStyle} type="number" value={wordLimit} onChange={(e) => setWordLimit(e.target.value)} placeholder="5000" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ ...labelStyle, fontSize: 10 }}>Figure Limit</label>
                <input style={smallInputStyle} type="number" value={figureLimit} onChange={(e) => setFigureLimit(e.target.value)} placeholder="6" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ ...labelStyle, fontSize: 10 }}>Table Limit</label>
                <input style={smallInputStyle} type="number" value={tableLimit} onChange={(e) => setTableLimit(e.target.value)} placeholder="4" />
              </div>
              <div style={{ flex: 1 }}>
                <label style={{ ...labelStyle, fontSize: 10 }}>Ref Limit</label>
                <input style={smallInputStyle} type="number" value={refLimit} onChange={(e) => setRefLimit(e.target.value)} placeholder="50" />
              </div>
            </div>
          )}
